...
const fullName = [];
const printFullName = students.forEach((person) => {
  if(person.period === 'Manhã'){
...
const findPerson = (name) => {
  const materias = students.find((person) => person.firstName === name).subjects;
   return materias.map((materia) => { 
    return `${materia.name} - ${(materia.grade >= 60) ? 'Aprovado' : 'Reprovado'}`;
  
  })
}

console.log(findPerson('Wilson'))